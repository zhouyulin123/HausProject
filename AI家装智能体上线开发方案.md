...
失败簇关闭约束：人工只能把失败簇推进到 `resolved`，不能直接填写 `verified`。存在待复测簇时，受保护评测环境必须显式提供目标 fingerprint 与修复版本；只有三组候选和基线证据完整、候选提交一致、目标在所有切分均未复发，才生成绑定提交与目标集合的签名复测证明。管理员通过质量页导入证明后由服务端验签并原子关闭全部目标；任一目标或版本不匹配则整批拒绝，后续复发会重开失败簇并清除旧验证来源。没有待复测目标时跳过该附加流程，不阻断常规发布证明。
