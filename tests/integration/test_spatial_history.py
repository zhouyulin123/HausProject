...
def test_draft_source_uses_owned_image_not_latest_version(context):
    from app.db.models import DesignTask, UploadedImage

    client, factory, url, headers, stranger = context
    ...
    with factory() as db:
        ...
        images = [
            UploadedImage(
                task_id=owner, file_url=f"/uploads/{i}.png", file_name=f"{i}.png"
            )
            for i, owner in enumerate([task_id, task_id, other.id])
        ]
        db.add_all(images)
        db.commit()
        ids = [image.id for image in images]
    for version, image_id in enumerate(ids[:2]):
        payload = request(version, f"source-{version}")
        payload["document"]["source_image_id"] = image_id
        assert client.put(url, headers=headers, json=payload).status_code == 200
    old = client.get(url + f"/sources/{ids[0]}", headers=headers)
    assert old.status_code == 200
    assert old.json()["image_id"] == ids[0]
    assert client.get(url + f"/sources/{ids[2]}", headers=headers).status_code == 404
    assert (
        client.get(
            url + f"/sources/{ids[0]}", headers={"X-Session-Id": stranger}
        ).status_code
        == 404
    )
...
def test_history_pagination_and_ownership(context):
    ...
    tail = client.get(
        url + "/versions?limit=2&before_version=2", headers=headers
    ).json()
...
def test_projection_never_invents_scale_or_room(context):
    ...
    assert (
        client.get(url + "/versions/1/rooms/r1/scene", headers=headers).status_code
        == 422
    )
    assert (
        client.get(url + "/versions/1/rooms/missing/scene", headers=headers).status_code
        == 404
    )
