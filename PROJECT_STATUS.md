...
1. 给商品库增加 GLB 模型 URL、真实三维尺寸、模型处理状态和许可来源字段。
2. 建立首批可商用 GLB 模型入库、压缩、预览和异常回退链路。
3. 在当前体块渲染器中按 SKU 加载 GLB，并保持体块作为模型缺失或加载失败时的明确降级。
4. 为 Scene Agent 增加摆放、移动、旋转、删除、碰撞和动线检查工具。
