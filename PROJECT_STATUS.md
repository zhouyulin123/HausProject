...
1. 增加生成任务异步执行与进度事件，避免模型调用占用同步请求
2. 为方案版本增加前端历史版本查看与“基于此版本继续优化”
...
1. 将节点执行记录拆成运行中事件，使前端能看到真实进度，而不只是最终版本轨迹
2. 引入持久化任务执行器，把同步模型请求改为可重试的后台任务
3. 增加预算上限条件分支：超限时自动调整商品组合或明确提示
4. 增加“基于历史版本继续优化”的工作流入口
