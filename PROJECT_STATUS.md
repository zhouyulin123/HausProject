...
- 新增迁移 `bf2a3b4c5d6e`，线性接续 `ae1f2a3b4c5d`；仓库真实案例的授权、标注与 `pending` 状态没有修改。
...
- revision 中的需求快照属于人工确认后的生成输入，不是 AI 需求解析或视觉预测；在独立预测快照进入证据链前，需求准确率、空间事实准确率和低置信确认率保持 `NO EVIDENCE`，不以确认输入制造虚假高分。
