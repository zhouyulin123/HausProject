...
- 新增独立的 `real_world_case_annotation/1.0` 标注契约与严格解析器，确定性覆盖需求事实、空间事实及置信度/待确认状态、允许 SKU、预算、布局硬约束、风格标签、人工维度评分和人工修改事实。
...
- RED `2fdabc0` 先定义缺失模块下的 48 项严格契约行为；GREEN 扩展为 49 项标注单元测试并全部通过，相关评测回归 132 项通过。
