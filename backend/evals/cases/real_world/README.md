...
公开仓库中的 `manifest.json` 是空模板，不保存真实案例、客户图片或标注。真实案例只在受控环境中通过治理流程冻结和读取。
...
证据包 6.0 使用独立 HMAC 密钥签名，绑定一个显式 split 的数据集内容指纹、匿名案例指纹、密钥域内匿名 `execution_ref`、运行终态、模型及三个运行时制品摘要，以及输入、真实预测、不可变输出和结果摘要。每条正式商品建议必须包含由目录硬过滤阶段生成的 `catalogEligibility/1.0`：资格策略、商品事实、判断时间和结论随方案与报价行一起进入不可变输出摘要；收集器只用这些冻结事实复算有效 SKU，不回查当前商品表，人工 `allowed_skus` 只计算商品匹配。缺失或互相矛盾时失败关闭。旧 3.0 的 `output_snapshot` 摘要不能作为不可变输出证据，旧 4.0 没有绑定真实模型预测，旧 5.0 没有分离有效 SKU 与人工商品匹配语义。Prompt 摘要从静态系统 Prompt、输出 Schema 和工具/调用参数契约复算；完整动态模型请求另行计算 `input_digest`，不得截断；规则摘要只绑定执行前可读取的生成与报价源码制品；数据摘要绑定模型实际接收的完整商品与定制价目上下文，不依赖成功后选出的方案。签发 CLI 不接受调用方自报版本。文件不包含案例 ID、数据库 task/run ID、资产路径、Prompt、输入、模型输出、原始预测或人工评审原文，只保留匿名引用及摘要。签名密钥必须只配置在受控 Worker/CI，不应写入仓库、命令行或开发者共享环境。
...
公开 trusted evidence `6.0` 的 execution 不包含数据库 `task_id` 或
...
必须读取同一份已验签 trusted evidence `6.0`，并只根据可信运行终态和
