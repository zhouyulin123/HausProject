...
- 当前全量验证：后端 870 项通过；前端 62 个测试文件共 252 项通过；TypeScript、Vite 生产构建、Python 编译、Ruff 与差异检查通过。构建仅保留 `vendor-three` 约 667 KB 的既有分包告警。
...
- 运营质量页已接入真实案例治理就绪度：20 个去重 `private_real` 案例且 development、regression、blind 三组均非空才满足门槛；只展示授权、标注、用途与分组聚合阻断，不泄露案例内容或资产路径。
