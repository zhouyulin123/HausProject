...
## 当前文档入口与工作阶段（2026-09-20）

- V2 已完成开发版功能闭环：整屋空间、通用家装编辑、冻结家具、空间变更影响、安装/预留/点位、受控 AI、清单比较、商品与材料估价、冻结交付、审阅和受控分享均已接通。已有物品和材料规则只作为开发代表案例，不改称正式真实案例。
- V2 完成后二次独立复审并修复发布门禁、私有缓存隔离、分享日志/异常、分享凭证碰撞、交付局部失败与开发依赖风险。当前最终回归基线为后端 1634 项通过、8 项跳过，前端 93 个测试文件共 485 项通过，TypeScript 与 Vite 7 生产构建通过；精确命令、浏览器旅程和边界见 V2 路线书 2026-09-20 记录。
- 当前本机统一启动器已运行 API、Generation、Effect Render、Blender 和 Frontend；体验入口为 `http://127.0.0.1:8080`，API `http://127.0.0.1:8081/ready` 已通过。
- 真实 `gpt-5.6-terra` 已在临时 SQLite 上完成四轮跨领域验收：授权商品加入、地面饰面新增、稳定对象移动和越权结构请求拒绝；34,355 tokens，实际记录成本 0.03483 CNY。桌面/手机同任务代表旅程完成空间、材料、AI、4,280 元估价、交付和分享。
- 材料估价显式绑定规则，冻结请求量、计费量、损耗、最低量、安装/运输/税和版本证据；规则写入使用期望版本与行锁，真实 MySQL 并发仅允许一个写入成功。未知或失效规则保持待报价。
- 大场景仅形成开发性能基线：桌面 200 件、手机 80 件单机单次冒烟通过，不是 p95 或生产 SLA。正式业务验收仍受真实授权住宅、商业价格审核、专业评审、20+ private_real 三分组、受保护发布、生产网关隐私和统计性能阻断。
- 本机开发数据库和 Alembic 代码均处于唯一 head `f6a7b8c9d0e1`。V2 首轮完整闭环已提交并推送为 `f67d060`；二次复审改动也已按阶段统一提交。最新证据和未完成门禁见 [V2 开发路线书](./V2开发路线书.md)。

- V1 已按开发版功能闭环收尾；启动、代码阅读、开发数据、验收边界及最新工作台记录统一见 [V1 项目阅读指南](./V1项目阅读指南.md)。
- V2 已获用户全权施工授权；主任务负责架构决策、接口契约、进度、界面设计方向、整合与最终验收，可以委派子任务施工与独立审查。常规产品和技术选择自主判断，执行与验收统一维护在 [V2 开发路线书](./V2开发路线书.md)。
- 用户明确并行施工只使用 Codex 子任务，不启动 Gemini。主任务分派互不重叠的后端与前端文件范围，独立复核后再记为通过。
- 阶段 2（路线 B）整屋空间底座已接通：原图描绘与尺度确认、多房间全局坐标、共享墙与门窗洞口、2D／3D、草稿恢复、撤销重做、不可变历史与精确单房间投影。编辑入口为设计工作台的“整屋户型”。空间专项 67 项、真实 MySQL 并发 2 项以及桌面／手机 Playwright 验收通过；完整证据及适用边界见 V2 路线书最新记录。
- 本机开发数据库已备份并升级至唯一 head `f6a7b8c9d0e1`；历史迁移与备份证据见 V2 路线书。
- 整屋架构采用任务级空间事实与既有单房间 SceneDocument 分离，禁止把独立居中缩放的房间直接拼接为可信整屋。详细 ADR、拟议接口和施工清单见 V2 路线书第 10 节。
- 12–20 周为 2026-09-16 的历史初步预算，不再代表剩余工期；当前开发版闭环已经完成，正式上线门禁依赖外部资料、评审和环境。
- 用户明确衣柜、厨房、家具摆放只是举例，不是 V2 的功能边界或优先级。后续先讨论完整家装能力与用户成果，再确定本期范围及代表场景，不沿用此前按示例固定的阶段顺序。
- 根目录仅保留上述两份主体文档；README 为简短入口，本文件保留协作上下文。后端专项运行说明、数据说明和技能文件继续保留，不新增并列计划书或独立状态长文。
- 旧状态文档中当时未提交的两项 2026-09-14 工作台记录已原样迁入 V1 指南；此前已跟踪的历史计划与记录可从 Git 历史追溯。
- 后续开发进展追加至 V2 路线书的维护记录，并在本文件更新简要状态。下方架构示例及旧日期记录属于历史上下文，当前实现以代码和 V1 指南为准；历史测试数量、数据库版本、商品和案例数量不代表本轮实时核验结果。
- 正式业务验收仍需真实授权案例、商业核验商品和受保护评测证据；开发版收尾不取消这些边界。
...
- `startHaus.bat` 改为单窗口管理：后端、三个 Worker 与前端以独立隐藏进程运行，启动完成后可关闭启动窗口；输出与错误日志分别写入 `backend/.runtime/launcher/`。
- 新增 `manageHaus.ps1` 和 `stopHaus.bat`，支持统一停止以及 `startHaus.bat --status`、`--stop`；记录 PID 与进程创建时间，停止时核对身份并结束子进程，防止 PID 复用误伤。
- 启停操作互斥，已运行服务不重复启动；部分服务退出时要求先停止再重启，端口冲突明确失败，启动失败回收本次已启动进程。前后端就绪且五个服务进程存活后才提示启动成功。
- Windows 隔离进程测试覆盖启动器退出后后台存活及持续写日志、中文空格路径、进程树停止、重复停止、PID 身份检查、启动互斥与提前退出检测；未重启用户正在运行的业务服务。旧版启动窗口需先手动关闭，之后使用新版脚本管理。
